...
% =========================================================================
    ...
    % words~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    [dictionary_words, wordle_words] = HW6_get_word_lists_Key();
    ...
    the_wordle_num = randi([1 200],1,1);    % this is the number of the
                                            %   chosen wordle word...
    ...
    the_wordle_word = char(wordle_words(the_wordle_num));    
    ...
    while ~is_game_over
        ...
        for col = 1:5
            ...
            [x,y] = HW6_get_valid_click_Key();
            %Get the clicked letter and store it in guess_boxes struct
            guessed_letter = HW6_get_the_letter_Key(x,y,keyboard);
        ...
        is_a_word = HW6_determine_if_guess_in_dictionary_Key(current_guess, dictionary_words);
        
        if ~is_a_word
            % If the current guess is not a real word, then we need to clear
            % the attempt:
            for col = 1:5
                guess_boxes(current_attempt_num,col).letter = '';
            end
            %And refresh the gameboard:
            HW6_refresh_game_board_Key(guess_boxes,keyboard)
            ...
            current_guess_colors = HW6_get_current_guess_colors_Key(current_guess,the_wordle_word);
            % Add the colors to the guess boxes and keyboard
            [guess_boxes,keyboard] = HW6_update_colors_in_structs_Key(guess_boxes,keyboard,current_attempt_num,current_guess_colors);
        
            %Check for game over and if the player is a winner
            [is_game_over, is_winner] = HW6_check_game_over_Key(current_guess, ...
                                                             the_wordle_word, ...
                                                             current_attempt_num);
