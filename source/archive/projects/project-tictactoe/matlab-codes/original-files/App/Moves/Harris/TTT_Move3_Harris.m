...
function move = TTT_Move3_Harris(board, player)
[n, m] = size(board);
ind = -1;
ind2 = -1;
ind3 = -1;
...
%     if board(i) == 0 && zeroAround(board, i)
    if ind ~= -1
        move = setFromTest(board, ind);
%         break
        return
    elseif ind2 ~= -1
        move = setFromTest(board, ind2);
% %         break
        return
    elseif ind3 ~= -1
        move = setFromTest(board, ind3);
%         break
        return
    elseif ind4 ~= -1
        move = setFromTest(board, ind4);
%         break
        return
    else
        for j = 1:9
