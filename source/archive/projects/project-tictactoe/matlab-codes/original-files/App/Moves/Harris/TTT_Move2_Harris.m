...
function move = TTT_Move2_Harris (board, player, randomSeed)
if nargin == 3
...
%     if board(i) == 0 && zeroAround(board, i)
    if ind ~= -1
        boardIndex = setFromTest(board, ind, randomSeed);
        break
    elseif ind2 ~= -1
        boardIndex = setFromTest(board, ind2, randomSeed);
        break
    else
        for j = 1:9
            if board(j) == 1 && zeroAround(board, j)
                ...
                break
            elseif j == 9
                boardIndex = pickRandom(board, randomSeed);
            end
...
function index = pickRandom(board, randomSeed)
rng(randomSeed)
    while true
        f = randi(9);
        if board(f) == 0
...
function index = setFromTest(board, ind, randomSeed)
if ind == 1 || ind == 2 || ind == 3
    for j = 1:3
        if board(j, ind) == 0
            boardIndex = ((ind-1)*3) + j;
            break
        end
    end
    board = pickRandom(board, randomSeed);
